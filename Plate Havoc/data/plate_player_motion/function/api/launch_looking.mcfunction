#> plate_player_motion:api/launch_looking
##
...
### Initialize
    ...
    data modify storage plate_player_motion:internal/temp matrix set value {x: 0.0d, y: 0.0d}
...
### Convert
    ...
    execute store result storage plate_player_motion:internal/temp convert.index int 1 run \
    ...
    execute store result score #z player_motion.internal.dummy store result storage plate_player_motion:internal/temp matrix.z double 1 run \
        function plate_player_motion:internal/convert_from_legacy/index with storage plate_player_motion:internal/temp convert
###
...
### `launch_local_xyz` flow
    ...
    ### If the player viewport angle is the same as the context angle, skip rotation calculation
        ...
        execute if score #equal_context player_motion.internal.dummy matches 1 \
            unless entity @s[x_rotation=-90] run return run function plate_player_motion:internal/launch/main
        ## Else, mitigate mojank's broken rotation math, pass the return value of `1` to indicate motion was applied
        execute if score #equal_context player_motion.internal.dummy matches 1 \
            run return run function plate_player_motion:internal/launch/handle_polar/local
    ###
    ...
    execute as d4bd74a7-4e82-4a07-8850-dfc4d89f9e2f in minecraft:overworld positioned 0.0 0.0 0.0 run \
        function plate_player_motion:internal/math/local_to_global with storage plate_player_motion:internal/temp matrix
    ...
    execute rotated as @s as d4bd74a7-4e82-4a07-8850-dfc4d89f9e2f in minecraft:overworld positioned 0.0 0.0 0.0 run \
        function plate_player_motion:internal/math/global/store_reference_vectors
    ...
    execute store result score #vec_k_combined player_motion.internal.dummy run \
        data get storage plate_player_motion:internal/temp vec_k[0] 10000
    execute store result score #temp1 player_motion.internal.dummy run \
        data get storage plate_player_motion:internal/temp vec_k[1] 10000
    execute store result score #temp2 player_motion.internal.dummy run \
        data get storage plate_player_motion:internal/temp vec_k[2] 10000
    ...
    execute if predicate plate_player_motion:internal/large_global \
        ...
        function plate_player_motion:internal/math/global/convert_large_to_local
    ...
    function plate_player_motion:internal/launch/main
###