...
data modify storage plate_havoc:ui bar.global.snbt[{id:players}].extra set value [{meta:default,text:"🛡",extra:[{selector:"@a[tag=plate_havoc.survivor,tag=plate_havoc.juggernaut]",color:red}," ",{atlas:"gui",sprite:"hud/heart/full"},{meta:value,storage:"plate_havoc:custom",nbt:jug_health,color:red,interpret:true}]}]
