##Active
data modify storage plate_havoc:cards active_entry set value {id:"plate_havoc:continue",visual:{body:[{text:"No Cards",color:gold},{text:"\nAll possible cards have been selected. The only way is to go forward."}],action:{label:[{metadata:name,text:"Continue"},{partition:votes,text:" "}],width:256}},non_card:true}
