$scoreboard players operation #Temp plate_havoc.temp = $(player) plate_havoc.player.damage.taken
$scoreboard players operation #Temp plate_havoc.temp += $(player) plate_havoc.player.damage.absorbed
execute if score #Temp plate_havoc.temp matches ..0 run return fail

data modify storage plate_havoc:temp temp_input set value {id:damage_taken,translate:"plate_havoc_content:leaderboard.player.damage_taken",fallback:"Damage Taken",extra:[": ",{meta:value,color:red}]}
