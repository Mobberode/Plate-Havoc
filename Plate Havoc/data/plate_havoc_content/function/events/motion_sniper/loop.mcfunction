execute unless entity @p[tag=plate_havoc.survivor,scores={plate_havoc_content.event.motion_sniper.time=1..}] run scoreboard players add #Motion_Sniper.CurrentDelay plate_havoc.event 1
...
execute as @a[tag=plate_havoc.survivor,scores={plate_havoc_content.event.motion_sniper.time=1..}] at @s run function plate_havoc_content:events/motion_sniper/player_tick
